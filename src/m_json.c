...
json_t *JS_Open(const char *lump, const char *type, version_t maxversion)
{
    ...
    {
        I_Error("%s: no version string", lump);
        JS_CloseOptions(lumpnum);
        return NULL;
    }
    ...
    {
        I_Error("%s: max supported version %d.%d.%d", lump,
