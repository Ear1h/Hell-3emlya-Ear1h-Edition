...
static boolean debug_stat_on;

static hu_textline_t w_visplanes;
static hu_textline_t w_segs;
static hu_textline_t w_vissprites;

static hu_stext_t w_custom;
static boolean print_on;
...
void HU_Start(void)
{
    ...
    HUlib_initSText(&w_custom, 60, 108, HU_MSGHEIGHT, hu_font, HU_FONTSTART,
                    &print_on);

    
    HUlib_initTextLine(&w_kills, 0, 160, hu_font, HU_FONTSTART);

    HUlib_initTextLine(&w_visplanes, 0, 144, hu_font, HU_FONTSTART);

    HUlib_initTextLine(&w_segs, 0, 152, hu_font, HU_FONTSTART);
    ...
    while (*p)
	HUlib_addCharToTextLine(&w_title, *(p++));
...
void HU_Ticker(void)
{
    ...
    if (print_counter && !--print_counter)
    {
        print_on = false;
    }
    ...
    // KIS Statistics
        ...
        M_snprintf(w_buffer, sizeof(w_buffer), "S %d/%d ", plr->secretcount,
                   totalsecret);
        s = w_buffer;
    ...
    if (debug_mode)
    {
        int plane_count = lastvisplane - visplanes;

        M_snprintf(plane_buffer, sizeof(plane_buffer), "PLN: %d/%d",
                   plane_count, MAXVISPLANES);
        HUlib_clearTextLine(&w_visplanes);
        t = plane_buffer;

        while (*t)
            HUlib_addCharToTextLine(&w_visplanes, *(t++));

        M_snprintf(plane_buffer, sizeof(plane_buffer), "SEGS: %d/%d",
                   ds_p - drawsegs, MAXDRAWSEGS);
        HUlib_clearTextLine(&w_segs);
        t = plane_buffer;

        while (*t)
            HUlib_addCharToTextLine(&w_segs, *(t++));
    }
