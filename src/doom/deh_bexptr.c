...
//
//
// Parses [POINTER] sections in BEX files
//
...
static void *DEH_BEXptr_Start(deh_context_t *context, char *line)
{
    char s[10];

    if (sscanf(line, "%9s", s) == 0 || strcmp("[CODEPTR]", s))
    {
        DEH_Warning(context, "Parse error in section start");
    }

    return NULL;
}

static void DEH_BEXParseLine(deh_context_t* context, char* line, void* tag)
{
    state_t *state;
