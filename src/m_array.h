...
inline static array_buffer_t* array_ptr(const void* v)
{
    return (array_buffer_t *) ((char *) v - offsetof(array_buffer_t, buffer));
}
...
#define array_delete_n(v, i, n)                                                                 \
    ...
    {                                                                                           \
        if (v)                                                                                  \
        {                                                                                       \
              memmove(&(v)[i], &(v)[i + n], sizeof(*(v)) * (array_ptr(v)->size - n - i));       \
              array_ptr(v)->size -= n;                                                          \
        }                                                                                       \
                                                                                                \
    } while (0)
...
inline static void *M_ArrayGrow(void* v, size_t ofsize, int n)
{
    array_buffer_t *p;
    ...
    {
        p = array_ptr(v);
        p = (array_buffer_t *) M_ARRAY_REALLOC(p, sizeof(array_buffer_t) 
                              + (p->capacity + n) * ofsize);
        p->capacity += n;
    }
    ...
    {
        p = (array_buffer_t *) M_ARRAY_MALLOC(sizeof(array_buffer_t) + n *
                                                    ofsize);
        p->capacity = n;
        p->size = 0;
    }
